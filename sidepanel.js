...
(function () {
  ...
  // Request header override (input panel)
  ...
  // Request header override (viewer toolbar)
  const viewerHeaderNameInput = document.getElementById('viewer-header-name-input');
  const viewerHeaderValueInput = document.getElementById('viewer-header-value-input');
  const btnApplyHeader = document.getElementById('btn-apply-header');
  ...
  function renderSwaggerUI({ url, spec, title }) {
    ...
    // Sync header inputs: prefer active override, then input-panel value
    if (activeHeaderName) {
      viewerHeaderNameInput.value = activeHeaderName;
      viewerHeaderValueInput.value = activeHeaderValue;
    } else if (headerNameInput.value.trim()) {
      viewerHeaderNameInput.value = headerNameInput.value.trim();
      viewerHeaderValueInput.value = headerValueInput.value;
      activeHeaderName = viewerHeaderNameInput.value;
      activeHeaderValue = viewerHeaderValueInput.value;
    }
    ...
    const doRender = (renderOpts) => {
      ...
      const headerName = activeHeaderName || headerNameInput.value.trim();
      const headerValue = activeHeaderName ? activeHeaderValue : headerValueInput.value;
      if (headerName) {
        opts.requestInterceptor = (req) => {
    ...
    if (url) {
      ...
      const headerName = activeHeaderName || headerNameInput.value.trim();
      const headerValue = activeHeaderName ? activeHeaderValue : headerValueInput.value;
      const fetchOptions = headerName ? { headers: { [headerName]: headerValue } } : undefined;
  ...
  function goBack() {
    ...
    viewerHeaderNameInput.value = '';
    viewerHeaderValueInput.value = '';
    updateServerHighlight();
    updateHeaderHighlight();
  }
  ...
  function updateServerHighlight() {
    if (viewerServerInput.value.trim()) {
      viewerServerInput.classList.add('override-active');
    } else {
      viewerServerInput.classList.remove('override-active');
    }
  }

  function updateHeaderHighlight() {
    if (viewerHeaderNameInput.value.trim()) {
      viewerHeaderNameInput.classList.add('override-active');
      viewerHeaderValueInput.classList.add('override-active');
    } else {
      viewerHeaderNameInput.classList.remove('override-active');
      viewerHeaderValueInput.classList.remove('override-active');
    }
  }
  ...
  // ===========================================================
  //  VIEWER HEADER OVERRIDE
  // ===========================================================

  btnApplyHeader.addEventListener('click', () => {
    activeHeaderName = viewerHeaderNameInput.value.trim();
    activeHeaderValue = viewerHeaderValueInput.value;
    updateHeaderHighlight();
    if (!lastLoadedSpec) return;
    renderSwaggerUI(lastLoadedSpec);
    showToast(activeHeaderName ? 'Request header applied' : 'Request header cleared', 'success');
  });
  ...
  btnResetHeader.addEventListener('click', () => {
    ...
    if (!lastLoadedSpec) return;
    renderSwaggerUI(lastLoadedSpec);
    showToast('Request header reset', 'success');
  });

  viewerHeaderNameInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') btnApplyHeader.click();
  });

  viewerHeaderValueInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') btnApplyHeader.click();
  });
